...
import { InconsistentUpdateParamsError, UnknownUpdateMethodError } from './errors';

/**
 * Represents a MongoDB query builder for constructing various types of queries.
 * @template DocumentType - The type of the MongoDB document.
 */
export class MongoQueryBuilders<DocumentType = unknown> implements QueryBuilders {
   ...
   */
  public buildUpdateQuery<UpdateType = unknown>(
    updates: UpdateType[],
    where: Where[],
    ...
    if (methodsSize > 1) {
      if ((methodsSize + updatesSize + whereSize) / 3 !== methodsSize) {
        throw new InconsistentUpdateParamsError(updatesSize, whereSize, methodsSize);
      }
   ...
   */
  public buildAggregationQuery(params: AggregationParams): MongoAggregateParams {
